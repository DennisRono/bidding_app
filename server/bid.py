...
@bid_router.get('/{id}', response_model=schemas.CreateUser, status_code=status.HTTP_200_OK)
...
def place_bid(bid: schemas.BidCreate, db: Session = Depends(get_db)):
    ...
    if datetime.utcnow() > product.end_time:
        raise HTTPException(
            ...
            detail="Bidding time has elapsed for this product"
        )
    
    
    last_bid = (
        db.query(models.Bids)
        .filter(models.Bids.product_id == bid.product_id)
        .order_by(models.Bids.bid_amount.desc())
        .first()
    )
